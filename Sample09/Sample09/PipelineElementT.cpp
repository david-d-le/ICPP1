...
PipelineElementT::PipelineElementT(int x, int y, IPipeline* pipeline) {
...
PipelineElementT::~PipelineElementT() {
}

bool PipelineElementT::IscorrectlyConnected(const IPipeline* pipeline) const {
	if (_x == 0 || _x == pipeline->_size - 1 || _y == pipeline->_size - 1
		...
		|| pipeline->_matrix[_x + 1][_y] == nullptr) {
		return false;
	}
	return true;
}